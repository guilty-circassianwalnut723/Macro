...
class MaxLongEdgeMinShortEdgeResize(torch.nn.Module):
    ...
    Args:
        ...
        max_pixels (int or list): Maximum pixels for the full image. Can be:
            - int: 使用原逻辑（max_pixels / img_num）
            - list: 根据 img_num 选择对应索引的值，类似 OmniGen 的 max_input_pixels 逻辑
    ...
    def _get_max_pixels_for_img(self, img_num):
        """
        根据 img_num 获取单张图片的 max_pixels 限制。
        
        - 如果 max_pixels 是 list，使用 max_pixels[img_num-1]（类似 OmniGen）
        - 如果 max_pixels 是 int，使用原逻辑 max_pixels / img_num
        """
        if isinstance(self.max_pixels, (list, tuple)):
            # 列表形式：根据图像数量选择对应的像素限制
            idx = min(img_num - 1, len(self.max_pixels) - 1)
            idx = max(0, idx)
            return self.max_pixels[idx]
        else:
            # 原逻辑：总像素除以图像数量
            return self.max_pixels / img_num

    def forward(self, img, img_num=1):
        """
        Args:
            img (PIL Image): Image to be resized.
            img_num (int): Number of images, used to determine max_pixels per image.
        Returns:
        ...
        # 获取当前图像数量对应的 max_pixels 限制
        current_max_pixels = self._get_max_pixels_for_img(img_num)

        # Ensure the number of pixels does not exceed max_pixels
        if new_width * new_height > current_max_pixels:
            scale = current_max_pixels / (new_width * new_height)
