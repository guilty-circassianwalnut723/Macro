    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            if 'image_transform_args' in dataset_args.keys():
                image_transform_args = dataset_args.pop('image_transform_args')
                
                # 检查是否有 max_input_pixels（用于输入图像的动态 pixels 限制）
                max_input_pixels = image_transform_args.pop('max_input_pixels', None)
                
                # 创建输入图像的 transform（使用 max_input_pixels 如果提供）
                if max_input_pixels is not None:
                    # 保存原始 max_pixels 用于输出图像（必须是 int，不能是 list）
                    output_max_pixels = image_transform_args.get('max_pixels')
                    if output_max_pixels is None:
                        raise ValueError("当指定 max_input_pixels 时，必须同时指定 max_pixels（用于输出图像）")
                    
                    # 输入图像使用 max_input_pixels（可以是 list 或 int）
                    input_transform_args = image_transform_args.copy()
                    input_transform_args['max_pixels'] = max_input_pixels
                    print(input_transform_args)
                    transform = ImageTransform(**input_transform_args)
                    
                    # 输出图像使用原始 max_pixels（必须是 int，不能是 list）
                    # 确保 output_max_pixels 是 int 类型
                    if isinstance(output_max_pixels, (list, tuple)):
                        # 如果 max_pixels 是列表，使用第一个值作为输出图像的像素限制
                        output_max_pixels = output_max_pixels[0] if len(output_max_pixels) > 0 else 589824
                        print(f"警告: max_pixels 是列表，输出图像将使用第一个值: {output_max_pixels}")
                    
                    output_transform_args = image_transform_args.copy()
                    output_transform_args['max_pixels'] = output_max_pixels
                    print(output_transform_args)
                    output_transform = ImageTransform(**output_transform_args)
                    dataset_args['output_transform'] = output_transform
                else:
                    transform = ImageTransform(**image_transform_args)
                
                dataset_args['transform'] = transform
            if 'vit_image_transform_args' in dataset_args.keys():
                vit_image_transform_args = dataset_args.pop('vit_image_transform_args')
                
                # 检查是否有 max_input_pixels（用于输入图像的动态 pixels 限制）
                max_input_pixels = vit_image_transform_args.pop('max_input_pixels', None)
                if max_input_pixels is not None:
                    vit_image_transform_args['max_pixels'] = max_input_pixels
                
                print(vit_image_transform_args)
                vit_transform = ImageTransform(**vit_image_transform_args)
            ...
            # 支持从配置文件直接指定数据路径（优先级高于DATASET_INFO）
            if 'data_dirs' in dataset_args.keys():
                # 直接从配置文件读取数据路径
                data_dirs = dataset_args.pop('data_dirs')
                if isinstance(data_dirs, str):
                    data_dirs = [data_dirs]
                dataset_args['data_dir_list'] = data_dirs
                
                # 处理parquet_info_path（如果提供）
                if 'parquet_info_path' in dataset_args.keys():
                    parquet_info_path = dataset_args.pop('parquet_info_path')
                    if isinstance(parquet_info_path, str) and os.path.exists(parquet_info_path):
                        if 'parquet_info' not in dataset_args.keys():
                            dataset_args['parquet_info'] = {}
                        with open(parquet_info_path, 'r') as f:
                            parquet_info = json.load(f)
                        dataset_args['parquet_info'].update(parquet_info)
                
                # 处理num_used_data（如果提供）
                if 'num_used_data' in dataset_args.keys():
                    num_used_data = dataset_args.pop('num_used_data')
                    if isinstance(num_used_data, int):
                        dataset_args['num_used_data'] = [num_used_data]
                    else:
                        dataset_args['num_used_data'] = num_used_data
                else:
                    # 如果没有指定，使用默认值（所有文件）
                    dataset_args['num_used_data'] = [len(dataset_args['data_dir_list'])]
            else:
                # 从DATASET_INFO读取（向后兼容）
