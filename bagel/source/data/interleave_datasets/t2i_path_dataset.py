...
class T2IPathIterableDataset(DistributedIterableDataset):
    """
    T2I数据集，从parquet中读取图像路径，然后从路径读取图像
    与T2IIterableDataset的区别是：不读取图像bytes，而是读取路径字符串
    这样可以大幅减小parquet文件大小，加快读取速度
    """
    def __init__(
                ...
                with fs.open_input_file(parquet_file_path) as f:
                    ...
                    for row_group_id in row_group_ids_:
                        ...
                        for row_idx, row in df.iterrows():
                            num_tokens = 0
                            try:
                                # 从parquet中读取图像路径（而不是bytes）
                                image_path = row['image_path']
                                
                                # 处理PyArrow标量类型，转换为字符串
