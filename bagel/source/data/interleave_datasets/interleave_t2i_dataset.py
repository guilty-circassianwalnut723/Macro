...
class InterleavedBaseIterableDataset(DistributedIterableDataset):
    ...
    def _add_image(self, data, image, need_loss, need_vae, need_vit, enable_cfg=True, img_num=1):
        """
        添加图像到数据中。
        ...
        Args:
            data: 数据字典
            image: PIL图像
            need_loss: 是否需要计算loss（输出图像）
            need_vae: 是否需要VAE编码（输入图像）
            need_vit: 是否需要ViT编码（输入图像）
            enable_cfg: 是否启用CFG
            img_num: 当前是第几张输入图像（用于动态max_pixels，1-indexed）
        """
        ...
        if need_loss:
            ...
            # 输出图像使用 output_transform（如果有）或 transform，不传 img_num
            if hasattr(self, 'output_transform') and self.output_transform is not None:
                image_tensor = self.output_transform(image)
            else:
        ...
        if need_vae:
            ...
            # 输入图像使用 transform，传入 img_num 用于动态 max_pixels
            image_tensor = self.transform(image, img_num=img_num)
            height, width = image_tensor.shape[1:]
            data['num_tokens'] += width * height // self.transform.stride ** 2
        ...
        if need_vit:
            ...
            vit_image_tensor = self.vit_transform(image, img_num=img_num)
...
class ParquetStandardIterableDataset(DistributedIterableDataset):
    ...
    def __init__(
        ...
        output_transform: optional transform for output images (uses different max_pixels).
        """
        super().__init__(dataset_name, local_rank, world_size, num_workers)
        self.transform = transform
        self.vit_transform = vit_transform
        self.output_transform = output_transform  # 用于输出图像的transform
    ...
    def __iter__(self):
        ...
        print(
            ...
            f"total {len(file_paths_per_worker)} files"
        )
