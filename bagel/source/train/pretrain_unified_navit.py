...
class TrainingArguments:
    ...
    sliding_window_step: int = field(
        default=1000,
        metadata={"help": "Save a temporary checkpoint every N steps."}
    )
    sliding_window_size: int = field(
        default=3,
        metadata={"help": "Keep at most N temporary checkpoints."}
    )


def delete_ckpt(ckpt_dir, step):
    path = os.path.join(ckpt_dir, f"{step:07d}")
    if os.path.exists(path):
        ...
        try:
            ...
            print(f"Failed to delete checkpoint {path}: {e}")


def rebuild_sliding_checkpoints(ckpt_dir, save_every, sliding_window_step, sliding_window_size, logger):
    """
    在训练恢复时重建 sliding_checkpoints 列表，并删除多余的滑动 checkpoint。
    
    滑动 checkpoint 的判断标准：
    - step % sliding_window_step == 0（是 sliding 保存点）
    - step % save_every != 0（不是永久保存点）
    
    Args:
        ckpt_dir: checkpoint 目录
        save_every: 永久保存间隔
        sliding_window_step: 滑动保存间隔
        sliding_window_size: 保留的滑动 checkpoint 数量
        logger: 日志记录器
    
    Returns:
        重建的 sliding_checkpoints 列表
    """
    if not os.path.exists(ckpt_dir):
        return []
    
    # 获取所有 checkpoint 目录（格式为 7 位数字）
    all_ckpts = []
    ...
    for step in all_ckpts:
        # 是滑动保存点但不是永久保存点
        if step % sliding_window_step == 0 and step % save_every != 0:
            sliding_ckpts.append(step)
    
    # 按 step 排序（从小到大）
    sliding_ckpts.sort()
    
    logger.info(f"[rebuild_sliding_checkpoints] 发现 {len(sliding_ckpts)} 个滑动 checkpoint: {sliding_ckpts}")
    
    # 如果超过窗口大小，删除最旧的
    to_delete = []
    while len(sliding_ckpts) > sliding_window_size:
        old_step = sliding_ckpts.pop(0)
        to_delete.append(old_step)
    
    if to_delete:
        logger.info(f"[rebuild_sliding_checkpoints] 需要删除 {len(to_delete)} 个旧的滑动 checkpoint: {to_delete}")
        for step in to_delete:
            threading.Thread(target=delete_ckpt, args=(ckpt_dir, step)).start()
    
    logger.info(f"[rebuild_sliding_checkpoints] 保留 {len(sliding_ckpts)} 个滑动 checkpoint: {sliding_ckpts}")
    return sliding_ckpts
...
def main():
    ...
    if dist.get_rank() == 0:
        ...
        # 获取wandb目录（优先使用环境变量WANDB_DIR，否则使用results_dir下的wandb目录）
        wandb_dir = os.environ.get('WANDB_DIR', os.path.join(training_args.results_dir, 'wandb'))
    ...
    if resume_from == training_args.resume_from:
    ...
    # 重建 sliding checkpoint 列表（在训练恢复时清理多余的滑动 checkpoint）
    sliding_checkpoints = rebuild_sliding_checkpoints(
        training_args.checkpoint_dir,
        training_args.save_every,
        training_args.sliding_window_step,
        training_args.sliding_window_size,
        logger
    )
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        save_permanent = (curr_step > 0 and curr_step % training_args.save_every == 0)
        save_sliding = (curr_step > 0 and curr_step % training_args.sliding_window_step == 0)

        if save_permanent or save_sliding:
            if dist.get_rank() == 0:
            ...
            if save_sliding and not save_permanent:
                sliding_checkpoints.append(curr_step)
                if len(sliding_checkpoints) > training_args.sliding_window_size:
                    to_delete = sliding_checkpoints.pop(0)
                    if dist.get_rank() == 0:
                        threading.Thread(target=delete_ckpt, args=(training_args.checkpoint_dir, to_delete)).start()
