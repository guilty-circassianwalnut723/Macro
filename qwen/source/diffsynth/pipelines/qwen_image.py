...
class QwenImageUnit_EditImageEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("edit_image", "tiled", "tile_size", "tile_stride", "edit_image_auto_resize", "max_input_pixels"),
    ...
    def _get_max_pixels_for_img_num(self, max_input_pixels, img_num):
        ...
        if isinstance(max_input_pixels, (list, tuple)):
            ...
            idx = max(0, idx)
            return max_input_pixels[idx]
        return max_input_pixels if max_input_pixels is not None else 1024 * 1024

    def edit_image_auto_resize(self, edit_image, max_pixels=None):
        max_pixels = max_pixels if max_pixels is not None else 1024 * 1024
        calculated_width, calculated_height = self.calculate_dimensions(max_pixels, edit_image.size[0] / edit_image.size[1])
        return edit_image.resize((calculated_width, calculated_height))


    def process(self, pipe: QwenImagePipeline, edit_image, tiled, tile_size, tile_stride, edit_image_auto_resize=False, max_input_pixels=None):
        if edit_image is None:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        if isinstance(edit_image, Image.Image):
            if edit_image_auto_resize:
                max_pixels = self._get_max_pixels_for_img_num(max_input_pixels, 1)
                resized_edit_image = self.edit_image_auto_resize(edit_image, max_pixels)
            else:
                resized_edit_image = edit_image
            ...
            total_images = len(edit_image)
            for idx, image in enumerate(edit_image):
                if edit_image_auto_resize:
                    max_pixels = self._get_max_pixels_for_img_num(max_input_pixels, total_images)
                    image = self.edit_image_auto_resize(image, max_pixels)
