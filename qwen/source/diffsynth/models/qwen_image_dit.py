...
def apply_rotary_emb_qwen(
    ...
    # Ensure contiguous memory layout before view_as_complex to avoid potential CUDA errors
    x_reshaped = x.float().reshape(*x.shape[:-1], -1, 2).contiguous()
    # Ensure contiguous memory layout before view_as_complex
    x_reshaped = x.float().reshape(*x.shape[:-1], -1, 2).contiguous()
    x_rotated = torch.view_as_complex(x_reshaped)
